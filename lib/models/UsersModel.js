...
UsersModel.prototype.add = function (name, socket) {
    // Check if the user is already logged in (from a different device)
    if (!this._users[name]) {
        // If not, create the user
        this._users[name] = new User(name, socket);
    } else {
        // If so, just update the socket reference
        // This avoids some weird bugs
        this._users[name].socket = socket;
    }
};
/**
 * Remove a user
...
/**
 * Disconnect any sockets with the same username as a user
 * @param: name - The username
 * @param: socket - The user's socket
 */
UsersModel.prototype.keepUserUnique = function (name) {
    ...
    for (oName in users) {
        if (users.hasOwnProperty(oName)) {
            if (oName === name) {
              
                // Get ref to other user
                oUser = users[oName];  
                
                oUser.socket.disconnect();
            }
        }
    }
};
...
UsersModel.prototype.setPending = function (nameCaller, nameCalled) {
    ...
    if (caller) 
        caller.pendingToUser = users[nameCalled];
};
/**
